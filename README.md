...
뱅크샐러드 거래 데이터를 분석하여 과소비 패턴을 감지하고 절약 기회를 제안하는 안드로이드 애플리케이션입니다.

## 📱 주요 기능

- **엑셀 데이터 업로드**: 뱅크샐러드에서 내보낸 거래 내역 엑셀 파일을 업로드하여 분석
- **과소비 패턴 분석**: 카테고리별, 시간대별, 반복 지출 패턴을 분석하여 과소비 구간 감지
- **절약 기회 제안**: 분석 결과를 바탕으로 구체적인 절약 방안 제시
- **소비 성향 분석**: 사용자의 소비 패턴을 분석하여 소비 성향 유형 분류
- **월별 통계**: 월별 지출 통계 및 트렌드 분석
...
### 앱 실행하기
1. 애뮬레이터 실행 : flutter emulators --launch [Emulator Name]
2. (cmd) BankDressing/test-app.bat [Emulator Name]
